...
public class LessonEnding extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        WordQueue queue = WordQueue.getInstance();
        if (queue == null) {
            WordQueue.initialize(this.getApplicationContext());
            queue = WordQueue.getInstance();
        }
        WordStatsDAO statsDAO = new WordStatsDAO(this.getApplicationContext());

        Log.v(this.getClass().getCanonicalName(), "Current position is " + Integer.toString(queue.GetPosition()));

        TextView word_position_text_view = (TextView) findViewById(R.id.activity_lesson_ending_position_number);
        word_position_text_view.setText(Integer.toString(queue.GetPosition()));
