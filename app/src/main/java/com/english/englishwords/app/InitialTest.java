    ...
    levelSubmitButton.setOnClickListener(new Button.OnClickListener() {
      public void onClick(View v) {
        ...
        Log.e(getClass().getCanonicalName(),
            "Learned words count: " + Integer.toString(LearningManager.getInstance().getLearnedWordsNum()));
        if (text.getText().toString().isEmpty()) {
          LearningManager.getInstance().pretendUserLearnedNFirstWords(0);
        } else {
          Log.e(getClass().getCanonicalName(),
              "Setting up " + Integer.toString(
                  Integer.parseInt(text.getText().toString())));
          LearningManager.getInstance().pretendUserLearnedNFirstWords(Integer.parseInt(text.getText().toString()));
        }
